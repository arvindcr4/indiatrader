...
class AdamManciniNiftyStrategy:
    ...
    def generate_signals(self, intraday: pd.DataFrame) -> pd.DataFrame:
        ...
        # Shift levels by one day forward (today's levels are based on yesterday's data)
        levels.index = levels.index + pd.Timedelta(days=1)
        open_range = intraday.resample("1D").apply(self._open_range)
        data = intraday.join(levels, how="left").ffill()
        data = data.join(open_range, rsuffix="_or", how="left").ffill()
